...
pub async fn run() -> Result<()> {
    ...
    check("data directory", data_dir.exists(), &data_dir.display().to_string());
    ...
    check("default policy", policy_file.exists(), &policy_file.display().to_string());

    let db_file = data_dir.join("findings.db");
    check("findings database", db_file.exists(), &db_file.display().to_string());
    ...
    let pcap_available = std::process::Command::new("which")
        ...
        .unwrap_or(false);
    check("libpcap (for net scanning)", pcap_available, "install with: brew install libpcap");
...
fn check(label: &str, ok: bool, detail: &str) {
    let icon = if ok { "✓" } else { "✗" };
    let note = if ok { detail.to_string() } else { format!("MISSING — {detail}") };
